...
sun.position.set(18, 28, 12); sun.castShadow = true; scene.add(sun);
...
function removeBlock(x,y,z,broadcast=false) {
  const k = key(x,y,z), mesh = blocks.get(k); if (!mesh) return;
  scene.remove(mesh); blocks.delete(k);
  if (broadcast) sendWorldAction({ kind:'removeBlock', x,y,z });
}
function generateWorld() {
  for (let x=-WORLD_SIZE/2; x<WORLD_SIZE/2; x++) {
    for (let z=-WORLD_SIZE/2; z<WORLD_SIZE/2; z++) {
      const h = Math.max(0, Math.floor((Math.sin(x*.45)+Math.cos(z*.38))*0.75));
      for (let y=-2; y<=h; y++) addBlock(x,y,z, y===h ? 'grass' : (y===h-1 ? 'dirt':'stone'));
    }
  }
}
function clearWorld() { for (const mesh of blocks.values()) scene.remove(mesh); blocks.clear(); }
...
function createPeer(id) {
  return new Promise((resolve,reject) => {
    ...
    peer.on('open', resolve); peer.on('error', reject);
  });
}
...
$('hostBtn').onclick = async () => {
  try {
    status.textContent='Creating room…'; roomId=randomRoom(); isHost=true; generateWorld();
    await createPeer(roomId); peer.on('connection', acceptConnection);
    startGame(`Room: ${roomId}`); toast(`Share room code ${roomId}`);
...
$('joinBtn').onclick = async () => {
  ...
  try {
    status.textContent='Joining room…'; await createPeer();
    const conn=peer.connect(`blockworld-${roomId}`, { reliable:true });
    conn.on('open',()=>{ connections.set(conn.peer,conn); wireConnection(conn); conn.send({kind:'hello',name:cleanName()}); startGame(`Room: ${roomId}`); });
...
function acceptConnection(conn) {
  connections.set(conn.peer, conn); wireConnection(conn);
...
function wireConnection(conn) {
  ...
  conn.on('close',()=>{ connections.delete(conn.peer); removeRemote(conn.peer); updatePlayerCount(); broadcast({kind:'playerLeft',id:conn.peer}); });
}
...
function updateRemote(p) {
  const id=p.id; if (!id || id===peer?.id) return;
  let entry=remotePlayers.get(id);
  if (!entry) {
    const group=new THREE.Group();
    const body=new THREE.Mesh(new THREE.BoxGeometry(.6,1.15,.35),new THREE.MeshLambertMaterial({color:0x3b82f6})); body.position.y=.85;
    const head=new THREE.Mesh(new THREE.BoxGeometry(.52,.52,.52),new THREE.MeshLambertMaterial({color:0xf0c8a0})); head.position.y=1.68;
    group.add(body,head); scene.add(group); entry={group,name:p.name||'Player'}; remotePlayers.set(id,entry);
  }
  entry.name=p.name||entry.name; entry.group.position.set(p.x,p.y-PLAYER_HEIGHT,p.z); entry.group.rotation.y=p.ry||0;
}
...
function interact(place) {
  ...
  const hits=raycaster.intersectObjects([...blocks.values()],false); if(!hits.length)return;
  const hit=hits[0], b=hit.object.userData;
  if(place){ const n=hit.face.normal; const x=b.x+n.x,y=b.y+n.y,z=b.z+n.z; if(Math.abs(camera.position.x-x)<.8 && Math.abs(camera.position.z-z)<.8 && Math.abs(camera.position.y-y)<1.8)return; addBlock(x,y,z,BLOCKS[selectedBlock].id,true); }
  else if(b.y>-2) removeBlock(b.x,b.y,b.z,true);
}
function buildHotbar(){ $('hotbar').innerHTML=''; BLOCKS.forEach((b,i)=>{const d=document.createElement('div');d.className=`slot ${i===selectedBlock?'selected':''}`;d.innerHTML=`<div class="swatch" style="background:#${b.colour.toString(16).padStart(6,'0')}"></div>`;d.onclick=()=>{selectedBlock=i;buildHotbar();};$('hotbar').append(d);}); }
...
function movePlayer(dt) {
  ...
  controls.moveRight(side*speed); if(collides(controls.object.position)) controls.object.position.copy(old);
  const old2=controls.object.position.clone(); controls.moveForward(forward*speed); if(collides(controls.object.position)) controls.object.position.copy(old2);
  if((keys.Space||keys.__jump)&&grounded){velocityY=7;grounded=false;keys.__jump=false;}
  velocityY-=18*dt; const beforeY=controls.object.position.y; controls.object.position.y+=velocityY*dt;
  if(collides(controls.object.position)){controls.object.position.y=beforeY;if(velocityY<0)grounded=true;velocityY=0;} else grounded=false;
...
function setupMobile(){
  ...
  joy.addEventListener('pointerdown',e=>joy.setPointerCapture(e.pointerId));
...
function animate(t){ requestAnimationFrame(animate); const dt=Math.min(clock.getDelta(),.05); if(started)movePlayer(dt);
  if(peer && t-lastNetworkSend>80){const p=controls.object.position;const msg={kind:'player',name:cleanName(),x:p.x,y:p.y,z:p.z,ry:controls.object.rotation.y};isHost?broadcast(msg):[...connections.values()][0]?.send(msg);lastNetworkSend=t;}
